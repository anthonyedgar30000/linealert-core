...
def test_physics_profile_uses_explicit_units_equations_and_boundaries():
    ...
    force = params["effective_imbalance_mass_kg"] * params["healthy_eccentricity_m"] * omega**2
...
def test_browser_kernel_contains_deterministic_physics_relationships():
    ...
    assert "real structure mass, stiffness, damping, mounting and resonance are not commissioned" in model
...
def test_event_model_emits_coupled_signature_before_concern_supporting_evidence():
    ...
    assert "Coupled rotational/tension signature moved outside synthetic healthy settling reference" in event_model
...
def test_plant_canvas_and_event_log_load_physics_before_event_model():
    ...
    assert plant.index("physics-informed-roll-model.js") < plant.index("plant-event-model.js")
    assert event_log.index("physics-informed-roll-model.js") < event_log.index("plant-event-model.js")
...
def test_event_log_profile_registers_signature_without_causal_claim():
    ...
    assert signature["boundary"] == "physics-informed model match != diagnosis or current root-cause proof"
