  ...
  return (
    <main className={styles.shell}>
      <header className={styles.header}>
        <div>
          <span className={styles.kicker}>LINEALERT · EVIDENCE VIEW</span>
          <h1>Current admitted machine conditions</h1>
          <p>
            Supporting evidence for investigation. This screen is deliberately behind the triage workflow;
            it does not determine ownership, root cause or equipment authority by itself.
          </p>
        </div>
      ...
      <section className={styles.statusStrip} aria-label="Current condition status">
        <div><span>STATION STATUS</span><b>{status}</b></div>
        <div><span>SOURCE MODE</span><b>{sourceMode}</b></div>
        <div><span>ADMITTED MEASUREMENTS</span><b>{runtime.payload?.measurement_count ?? 0}</b></div>
        <div><span>REFUSALS</span><b>{runtime.payload?.refusal_count ?? 0}</b></div>
      </section>

      <section className={styles.workspace}>
        <div className={styles.sectionHeading}>
          <div><span>ACTIVE EVIDENCE</span><h2>Relationships requiring operator attention</h2></div>
          <small>Latest admitted measurement per relationship</small>
        </div>

        {runtime.state === "loading" && <div className={styles.emptyState}>Reading the current condition stream…</div>}
        {runtime.state === "unavailable" && (
          <div className={styles.emptyState}>
            Condition evidence is unavailable. Do not substitute a training case or commissioning scenario for live or retained machine evidence.
          </div>
        )}
        {runtime.state === "active" && latestConditions.length === 0 && (
          <div className={styles.emptyState}>
            No admitted condition measurement is active. Machine Health remains available for retained history.
          </div>
        )}
            ...
            {latestConditions.map((observation) => {
              ...
              return (
                <article className={`${styles.conditionCard} ${outside ? styles.attention : ""}`} key={observation.relationship_id ?? observation.signal}>
                  ...
                  {outside && <OperatorActions observation={observation} sourceMode={sourceMode} />}
                  <Link className={styles.historyLink} href="/health">Review retained condition history →</Link>
                </article>
              );
            })}
          </div>
      ...
      <section className={styles.boundary}>
        <div>
          ...
          <b>Evidence supports triage and investigation; it does not silently become diagnosis.</b>
        </div>
        <p>
